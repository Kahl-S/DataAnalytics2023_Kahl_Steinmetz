...
# ALTERATIONS MADE TO THIS FILE BY KAHL STEINMETZ CAN BE SEEN BELOW
...
def get_query(embedding_type, query_embedding, date_range):
    ...
    if len(date_range) > 0:
        ...
        if len(date_range) > 1:
            additional_filters[-1]["range"]["created_at"]["lte"] = date_range[1].strftime(
                "%Y-%m-%d")

# Query must be modified to include features of interest such as 'created_at' and 'sentiment.roberta.primary'
    query = {
        "_source": ["id_str", "text", "extended_tweet.full_text", "quoted_status.text",
                    ...
                    "sentiment.roberta.primary", "user.verified", "user.description", "created_at"],
        "query": {
            "script_score": {
                "query": {
                    "bool": {
...
def run_query(es_uri, es_index, embedding_type, embedding_model, query, date_range, max_results=1000):
    ...
    with Elasticsearch(hosts=[es_uri], timeout=60, verify_certs=False) as es:
        ...
        s.update_from_dict(
            get_query(embedding_type, query_embedding, date_range))

# Appended these entries to esstially declare the variables to be recorded before being recorded by the hit loop
        tweet_text = []
        tweet_text_display = []
        tweet_embeddings = []
        tweet_scores = []
...
# Modified the hit loop to grab the variables of interest that have been declared above and in the
# 'get_query' function
        for hit in s.execute():
            tweet_embeddings.append(
                np.array(hit["embedding"][embedding_type]["primary"]))
...
#Begin Kahl's hit loop entries
            sbert_val.append(hit['sentiment']['roberta']['primary'])
            ver_bool.append(hit['user']['verified'])
            tweet_ids.append(hit['id_str'])
            create_date.append(hit['created_at'])
...
# Return modified to return the variables of interest, note that some original returns
# have been removed to increase speed and decrease memory impact
        return create_date, sbert_val, ver_bool, tweet_scores, tweet_ids
...
def get_index_date_boundaries(es_uri, es_index, embedding_type):
    ...
    min_date = datetime.strptime(
        results.aggregations.min_date.value_as_string, "%Y-%m-%d").date()
    max_date = datetime.strptime(
        results.aggregations.max_date.value_as_string, "%Y-%m-%d").date()
...
def compute_aspect_similarities(tweet_embeddings, embedding_type, embedding_model, aspects):
    ...
    if embedding_type == "sbert":
        aspect_embeddings = embedding_model.encode(
            aspects, normalize_embeddings=True)
    elif embedding_type == "use_large":
...
def cluster_vectors(vectors, clustering_type, kmeans_n_clusters,
    ...
    if dreduce_dim is not None and dreduce_dim < vectors.shape[-1]:
        dreduce_umap = umap.UMAP(
            n_neighbors=30, n_components=dreduce_dim, min_dist=0.0)
    ...
    if clustering_type == "kmeans":
        if kmeans_n_clusters == 1 or vectors.shape[0] == 1:
            ...
            if kmeans_n_clusters > 0:
                ...
                kmeans = cluster_helpers.detect_optimal_clusters(
                    vectors, plot_elbow=True)
                elbow_plot = plt.gcf()
    ...
    silhouette_score = cluster_helpers.get_silhouette_score(
        vectors, cluster_assignments)
